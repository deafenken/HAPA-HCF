...
from models.self_attention import Transformer

class Temporal(Module):

    def __init__(self, input_size, out_size):
        super(Temporal, self).__init__()
        self.conv_1 = nn.Sequential(nn.Conv1d(in_channels=input_size, out_channels=out_size, kernel_size=3, stride=1, padding=1), nn.ReLU())
...
class UnimodalMILHead(nn.Module):

    def __init__(self, input_dim, h_dim=512, dropout_rate=0.0):
        super(UnimodalMILHead, self).__init__()
...
class Unimodal(Module):
    ...
    def __init__(self, input_size, h_dim=512, feature_dim=64):
        ...
        self.mil = UnimodalMILHead(input_dim=feature_dim, h_dim=h_dim)
